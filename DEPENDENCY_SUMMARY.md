...
| `copy_dependencies.sh` | 依赖复制 | 递归复制所有第三方库到 vendor |
| `create_symlinks.sh` | 创建符号链接 | 为版本号库文件创建简化链接 |
| `fix_rpath.sh` | 路径修复 | 将绝对路径改为 @rpath 相对路径 |
| `verify_vendor.sh` | 验证配置 | 检查依赖完整性和路径正确性 |
