...
- [x] 修改 `videoPlayerApp.ts` 使用领域模型
- [x] 使用 `ApplicationService` 替代直接操作
- [x] 保持向后兼容（单一 `playlist` + `getList`/`setList` 等方法）

#### 步骤 5.2: 重构 corePlayer
- [x] 修改 `corePlayer.ts` 使用 `MediaPlayer` 接口
- [x] 内部实现使用 `MpvMediaPlayer`（含 `setExternalController`）
- [x] 保持接口兼容

#### 步骤 5.3: 重构 playerState
- [x] 修改 `playerState.ts` 使用 `PlaybackSession`
- [x] 保持事件兼容性（`sessionToPlayerState` 内联）
...
#### 步骤 7.1: 移除旧代码
- [x] 确认所有功能已迁移
- [x] **移除阶段 5–6 引入的过渡性适配层**：删除 `adapters/PlayerStateAdapter`、`adapters/PlaylistAdapter`；`sessionToPlayerState`/`phaseToStatus` 内联至 `playerState.ts`，播放列表为单一 `playlist`（`Playlist`）+ `getList`/`setList` 等方法在 `videoPlayerApp`
...
### 2026-01-26 - 重构完成
- **阶段 1–4**：领域模型、适配器、MpvMediaPlayer、应用层（commands/queries）已就绪
- **阶段 5**：videoPlayerApp、corePlayer、playerState 已迁移至领域模型与 ApplicationService
- **阶段 6**：IPC 层已切换为 commands/queries（pause/resume/seek/volume/get-playlist 等）
- **阶段 7**：过渡性适配层已移除，`sessionToPlayerState` 内联至 playerState，单一 `playlist` + 方法收敛至 videoPlayerApp
...
- **2026-01-25**: 初始规划文档创建
- **2026-01-26**: 阶段 1–7 执行完成，规划状态更新为「已完成」
